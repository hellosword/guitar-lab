...
### 移动端练习体验打磨与 PWA 发布演练
- **来源**：本次对话（2026-05-04），在产品结构稳定后评估下一步方向
- **内容**：
  - 检查手机竖屏、手机横屏、平板横屏下的练习页、图选弹窗、指板速查、弱点地图
  - 优化指板点击热区、按钮尺寸、右侧详情在窄屏下的折叠/排序
  - 验证音频播放、PWA 缓存更新、版本号/构建标识在移动端是否可靠
  - 做一次“给真实用户打开就能练”的发布演练，不急于上架应用商店
- **预期产出**：移动端交互问题清单、对应修复、PWA 发布检查清单
...
| 2026-05-04 | 待发布 | 移动端第一轮交互打磨：指板图在窄屏中保持更大的可点击宽度并在卡片内横向滚动，图选通路图改为窄屏内部横向滚动，补充页面横向溢出回归测试 | `src/components/Fretboard/index.tsx`、`src/App.tsx`、`tests/e2e/mvp-practice.spec.ts` |

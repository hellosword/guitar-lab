...
async function openMobileSettings(page: Page): Promise<void> {
  if (isMobileViewport(page)) {
    ...
    await page.getByRole('button', { name: '展开练习通路选择' }).click();
    await expect(page.getByRole('dialog', { name: '练习通路' })).toBeVisible();
  }
}

async function closeMobileSettings(page: Page): Promise<void> {
  const settingsDialog = page.getByRole('dialog', { name: '练习通路' });
...
test('MVP 练习页可见并能完成一道音名题', async ({ page }) => {
  ...
  if (isMobile) {
    await expect(page.getByRole('button', { name: '展开练习通路选择' })).toBeVisible();
    await expect(page.getByRole('tab', { name: '练习' })).toHaveAttribute('aria-selected', 'true');
    await expect(page.getByRole('tab', { name: '弱点' })).toBeVisible();
...
test('组内混合不会抽到组外题型且六线谱不提供混合', async ({ page }) => {
  ...
  if (isMobileViewport(page)) {
    ...
    await openMobileSettings(page);
    await expect(page.getByRole('dialog', { name: '练习通路' }).getByRole('button', { name: '混合', exact: true })).toHaveCount(0);
    await closeMobileSettings(page);
  } else {

...
async function openWeaknessView(page: Page): Promise<void> {
  if (isMobileViewport(page)) {
    await page.getByRole('button', { name: '弱点' }).click();
    return;
  }
...
async function selectSolfeggioDisplay(page: Page, label: 'Do Re Mi' | '1 2 3'): Promise<void> {
  if (isMobileViewport(page)) {
    await openMobileSettings(page);
    await page.getByRole('dialog', { name: '练习设置' }).getByRole('button', { name: label }).click();
    await closeMobileSettings(page);
    return;
  }
...
test('MVP 练习页可见并能完成一道音名题', async ({ page }) => {
  ...
  if (isMobile) {
    await expect(page.getByRole('heading', { name: '综合练习' })).toBeVisible();
  ...
  if (isMobile) {
    await expect(page.getByText('G 大调 · 0-4 品', { exact: true })).toBeVisible();
  } else {
  ...
  if (isMobile) {
    await openMobileSettings(page);
    const settingsDialog = page.getByRole('dialog', { name: '练习设置' });
    await expect(settingsDialog.getByRole('button', { name: '综合' })).toBeVisible();
    await expect(settingsDialog.getByRole('button', { name: '指板记忆' })).toBeVisible();
    await expect(settingsDialog.getByRole('button', { name: '位置唱名' })).toHaveCount(0);
    await expect(settingsDialog.getByRole('button', { name: '图选通路' })).toBeVisible();
    await settingsDialog.getByRole('button', { name: '指板记忆' }).click();
  } else {
  ...
  if (isMobile) {
    await openMobileSettings(page);
    const settingsDialog = page.getByRole('dialog', { name: '练习设置' });
    await expect(settingsDialog.getByText('音名', { exact: true }).first()).toBeVisible();
    await expect(settingsDialog.getByText('唱名', { exact: true }).first()).toBeVisible();
    await expect(page.getByLabel('选择当前调')).toHaveCount(0);
    await expect(settingsDialog.getByRole('button', { name: '位置 -> 唱名' })).toBeVisible();
    await expect(settingsDialog.getByRole('button', { name: '唱名 -> 位置' })).toBeVisible();
    await settingsDialog.getByRole('button', { name: '图选通路' }).click();
  } else {
...
test('组内混合不会抽到组外题型且六线谱不提供混合', async ({ page }) => {
  ...
  await selectDirection(page, '音名唱名', '混合');
  ...
  if (isMobileViewport(page)) {
    await openMobileSettings(page);
    const settingsDialog = page.getByRole('dialog', { name: '练习设置' });
    await settingsDialog.getByRole('button', { name: '六线谱', exact: true }).click();
    await openMobileSettings(page);
    await expect(page.getByRole('dialog', { name: '练习设置' }).getByRole('button', { name: '混合', exact: true })).toHaveCount(0);
    await closeMobileSettings(page);
  } else {
...
test('全局唱名显示模式会同步影响练习、指板记忆和弱点地图', async ({ page }) => {
  ...
  await selectSolfeggioDisplay(page, '1 2 3');
  await selectDirection(page, '音名唱名', '音名 -> 唱名');
  ...
  if (isMobileViewport(page)) {
    await openMobileSettings(page);
    await expect(page.getByRole('dialog', { name: '练习设置' }).getByRole('button', { name: '1 2 3' })).toHaveAttribute('aria-pressed', 'true');
    await closeMobileSettings(page);
  } else {
